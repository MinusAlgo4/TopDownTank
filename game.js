...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let score = 0;
  let highScore = Number(localStorage.getItem("tank_highscore_v1") || 0);
    ...
    for (const tnk of targets) {
      ...
      if (hit) {
        ...
        if (!b.owner.isEnemy) {
          ...
          if (score > highScore) {
            highScore = score;
            localStorage.setItem("tank_highscore_v1", String(highScore));
          }
        }
  ...
  function step(now) {
    ...
    if (running) {
      ...
      if (!player.alive && !gameWon) {
        ...
        document.querySelector(".overlay-sub").textContent =
            "You died at Wave " + currentWave + ". Score: " + score;
        startBtn.textContent = "PLAY AGAIN";
      }
    ...
    if (uiHighScore) uiHighScore.textContent = String(highScore);
